...
let answered = false; // ใช้เช็คว่าข้อปัจจุบันตอบไปแล้วหรือยัง (ใช้ได้ทั้งแบบเลือกและแบบพิมพ์)
...
const POINTS_PER_QUESTION = 0.5454545454545455;
...
// เช็คว่าข้อนี้เป็นแบบพิมพ์คำตอบ (fill-in-the-blank) หรือแบบเลือกตัวเลือก
function isFillBlankQuestion(q) {
    return q.type === 'fill_blank' || (!q.options && (q.answer !== undefined));
}
...
function renderQuestion() {
    ...
    if (isFillBlankQuestion(q)) {
        // --- โหมดพิมพ์คำตอบ ---
        optionsContainer.classList.add('hidden-step');
        textAnswerContainer.classList.remove('hidden-step');

        const input = document.getElementById('text-answer-input');
        input.value = '';
        input.disabled = false;
        input.className = "w-full border-2 border-orange-200 rounded-xl p-4 outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-200 transition text-lg";

        const submitBtn = document.getElementById('submit-text-btn');
        submitBtn.disabled = false;
        submitBtn.className = "bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 px-8 rounded-xl shadow-md transition transform hover:scale-105 whitespace-nowrap";

        const feedbackEl = document.getElementById('text-answer-feedback');
        feedbackEl.classList.add('hidden');
        feedbackEl.innerText = '';

        // ให้เคอร์เซอร์ไปโฟกัสที่ช่องพิมพ์ทันที เพื่อให้พิมพ์ต่อได้เลย
        setTimeout(() => input.focus(), 50);
    } else {
        // --- โหมดเลือกตัวเลือก (แบบเดิม) ---
        textAnswerContainer.classList.add('hidden-step');
        optionsContainer.classList.remove('hidden-step');
...
// --- ระบบพิมพ์คำตอบ (Fill-in-the-blank) ---
function normalizeAnswer(str) {
    return String(str || '')
        .trim()
        .toLowerCase()
        .replace(/\s+/g, ' '); // ยุบช่องว่างซ้ำให้เหลืออันเดียว
}
...
function submitTextAnswer() {
    ...
    if (userAnswerRaw.trim() === '') {
        input.focus();
        return; // ยังไม่ได้พิมพ์อะไรเลย ไม่ให้ส่ง
    }

    // รวบรวมคำตอบที่ยอมรับได้ทั้งหมด (คำตอบหลัก + คำตอบสำรองถ้ามี)
    const acceptedAnswers = (q.accepted_answers && q.accepted_answers.length > 0)
        ? q.accepted_answers
        : [q.answer];

    const normalizedUser = normalizeAnswer(userAnswerRaw);
    const isCorrect = acceptedAnswers.some(a => normalizeAnswer(a) === normalizedUser);

    answered = true;

    // 1. ล็อกช่องพิมพ์และปุ่มส่ง
    input.disabled = true;
    const submitBtn = document.getElementById('submit-text-btn');
    submitBtn.disabled = true;
    submitBtn.className = "bg-gray-300 text-gray-500 font-bold py-3 px-8 rounded-xl cursor-not-allowed transition whitespace-nowrap";

    input.classList.remove('border-orange-200', 'focus:border-orange-500');
    input.classList.add(isCorrect ? 'border-green-500' : 'border-red-500', 'bg-gray-50');

    // 2. โชว์ผลเฉลยใต้ช่องพิมพ์
    const feedbackEl = document.getElementById('text-answer-feedback');
    feedbackEl.classList.remove('hidden');
    if (isCorrect) {
        feedbackEl.className = "text-sm font-medium text-green-600";
        feedbackEl.innerText = `✅ ถูกต้องครับ! คำตอบคือ "${q.answer}"`;
    } else {
        feedbackEl.className = "text-sm font-medium text-red-600";
        feedbackEl.innerText = `❌ ไม่ถูกต้อง คุณตอบ "${userAnswerRaw}" (เฉลย: "${q.answer}")`;
    }
    ...
    if (isCorrect) {
        ...
        missedQuestions.push({
            question: q.question,
            userAns: userAnswerRaw,
            correctAns: q.answer,
            explanation: q.explanation
        });
    }

    // 4. แสดงคำอธิบาย (Explanation) เหมือนโหมดเลือกตัวเลือก
    const hintDisplay = document.getElementById('hint-display');
    hintDisplay.classList.remove('hidden', 'bg-yellow-50', 'border-yellow-200', 'text-yellow-800');
    if (isCorrect) {
        hintDisplay.classList.add('bg-green-50', 'border-green-200', 'text-green-800');
    ...
    // 5. เปิดปุ่ม "ข้อถัดไป" ให้กดไปต่อได้
    const nextBtn = document.getElementById('next-btn');
    nextBtn.disabled = false;
    nextBtn.className = "bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 px-8 rounded-xl shadow-md transition transform hover:scale-105";
}
